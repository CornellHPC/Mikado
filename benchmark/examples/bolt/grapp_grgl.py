"""Standalone grapp + cuSPARSE BOLT-LMM-inf benchmark. Using pygrgl as backend.

Loads one or more `.grg` files, runs `bolt_lmm_inf` against a synthetic
...
def main():
    ...
    grgs = [
        load_grg_calculator(path) for path in path_strs
    ]
    ...
    LOGGER.info("h2=%.4f  sigma_g2=%.4f  calibration=%.4f  variants=%d",
                fit.h2, fit.sigma_g2, cal.factor, len(df))
