"""Standalone grapp + cuSPARSE PCA benchmark, LOBPCG solver.

LOBPCG variant of ``grapp_cusparse.py``. Loads one or more `.grg_spmv` files (or
a directory searched for `*.grg_spmv`), combines them into a single
...
-------------------------------
LOBPCG's ``tol`` is a **residual-norm** tolerance (``||A x - lambda x|| <= tol``),
applied identically by the SciPy (host) and CuPy (GPU) implementations. So an
explicit ``--tol > 0`` is directly comparable across grgl / mkl / cusparse and
across the ``--native`` (GPU CuPy LOBPCG) vs non-native (host SciPy LOBPCG with
GPU SpMV) paths.

The only divergence is the *default* used when ``tol <= 0``:
  * non-native (SciPy host): ``sqrt(eps_float64) * n ~= 1.49e-8 * n``
  * native (CuPy GPU):       ``sqrt(1e-15) * n     ~= 3.16e-8 * n`` (~2.1x looser)
where ``n`` is the number of individuals. Prefer an explicit ``--tol`` (or
``--tol-record`` against an MKL run) for apples-to-apples error across backends.
``maxiter`` defaults to 100 because the solver default of 20 under-converges PCA.
"""
...
LOGGER = logging.getLogger("grapp_cusparse_lobpcg")
...
def main():
    ...
    ap.add_argument("--tol", type=float, default=0.0,
                    help="LOBPCG residual-norm tolerance passed to PCs. 0 uses the solver "
                         "default (sqrt(eps)*n on the SciPy host path, sqrt(1e-15)*n on the "
                         "CuPy native path). Default: 0.")
    ...
    ap.add_argument("--native", action="store_true",
                    help="Enable GPU-native I/O (requires --capture). Runs fully GPU-resident "
                         "LOBPCG via PCs on a CuPy-backed GRG; without it LOBPCG runs on the "
                         "host (SciPy) with GPU SpMV (host transfers per matmul).")
    ...
    req = make_runconfig_pca(force_spmm=args.force_spmm, maxk=args.pcs)
    ...
    with ExitStack() as stack:
        ...
        if args.native:
            # Native runs do LOBPCG on the GPU (cupyx lobpcg), so init_matrix must
            # be a device array; PCs passes init_matrix through to lobpcg unchanged.
            init_matrix = cupy.asarray(init_matrix)
    ...
    # LOBPCG residual tolerance actually in effect when --tol <= 0. The native path
    # runs CuPy lobpcg (sqrt(1e-15)*n); the non-native path runs SciPy lobpcg on the
    # host (sqrt(eps_float64)*n).
    effective_tol = args.tol
    if args.tol <= 0.0:
        if args.native:
            effective_tol = numpy.sqrt(1e-15) * n_individuals
        else:
