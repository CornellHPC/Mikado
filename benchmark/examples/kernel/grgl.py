"""Standalone pygrgl matmul kernel benchmark.

Loads one or more `.grg` files (or a directory searched for `*.grg`) directly
via pygrgl (no grapp), then times only the sparse matmul kernel: a concurrent
sweep that dispatches one `pygrgl.matmul` per chromosome across a ThreadPool and
measures the wall time of the whole sweep. Loading time is excluded.
...
LOGGER = logging.getLogger("kernel_grgl")
...
def main():
    ...
    # --- Load (excluded from timing). load_up_edges=True is NOT required for 'up' and will harm performance. ---
    t_load0 = time.perf_counter()
    grgs = [pygrgl.load_immutable_grg(p) for p in path_strs]
    ...
    def do_matmul(grg, v):
        return pygrgl.matmul(grg, v, direction)
